...
import { apiBulkDeleteEmployee } from '@/services/EmployeeService'
...
const EmployeeListSelected = () => {
    ...
    const handleConfirmDelete = async () => {
        try {
            const ids = selectedEmployee.map(employee => employee.id).filter((id): id is string => Boolean(id))
            
            await apiBulkDeleteEmployee(ids)
            
            toast.push(
                <Notification type="success">
                    Successfully deleted {ids.length} employee(s)!
                </Notification>,
    ...
    return (
        <>
            ...
            >
                <p>
                    Are you sure you want to remove {selectedEmployee.length} employee(s)? This action
                    cannot be undone.
                </p>
            </ConfirmDialog>
            <Dialog
            ...
            >
                ...
                <p>Send message to the following employees</p>
                <Avatar.Group
                    chained
                ...
                >
                    {selectedEmployee.map((employee) => {
                        ...
                        return (
                            <Tooltip key={employee.id} title={fullName}>
                                <Avatar size={30} src={employee.avatar || undefined} alt={fullName} />
                            </Tooltip>
                        )
                    })}
                </Avatar.Group>
